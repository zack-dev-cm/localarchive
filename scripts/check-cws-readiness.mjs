...
async function validatePermissions() {
  ...
  for (const permission of ["storage", "tabs", "history", "management", "cookies"]) {
...
async function validateListingUrls() {
  ...
  assert(listing.source_url.startsWith("https://github.com/"), "source_url should keep pointing at the public source repo");
}
...
async function validatePrivacyPolicy() {
  ...
  assert(!policy.includes("after the public repository is created"), "privacy policy contains stale support placeholder");
}

async function validateSiteSource() {
  const pages = [
    ["site/index.html", "LocalArchive"],
...
async function validateLiveUrls() {
  ...
  for (const field of ["official_url", "support_url", "privacy_policy_url", "reviewer_instructions_url"]) {
    ...
    assert(text.includes("LocalArchive"), `${field} live page does not contain LocalArchive`);
  }
}
